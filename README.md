...
- [ ] videomae-large的微调（把能用的都用了，mafw，dfew）- 何艺超
- [ ] hubert的情感数据集微调，多个分类头，把数据混合去微调 - 子健
- [x] mac-bert的情感数据集微调 具有转录的数据集有哪些? (没找到具有转录文本的数据集，那么考虑到转录质量本身得不到保障，且我们人手也不够，那么只能放弃)
- [ ] 用cy给的那个东西，去提特征 - 程浩
- [ ] 任何一个模型微调完都需要提取两个级别的特征，提取完特征就马上去baseline上实现测试效果，去test上测试
- [ ] 以上微调部分的调参由于时间关系，只对学习率进行考量
- [ ] 写一个***leaderboard***，把每个模态能够达到的最好的特征都同步（考虑到之前的经验，一般特征在val上好，在test上就会好，但为了排除特殊性，取得阶段性成果之后仍然要去test上实验

特征融合：
- [ ] backbone部分，张龙的融合机制（必须得是fra级别的特征，在提取特征的时候也得考虑一下）- 张龙
- [ ] 三模态的融合，紧急需要实现 - 张龙
